...
std::string default_style() {
    return R"xxx(
body { margin:0em; padding:0em; padding-top:2em; font-size:calc(1vw + 14pt); }
h1 { text-align:center; margin: 0em 0em; font-size:175%; }
h2 { margin: 0.2em 0; font-size:125%; }
h3 { margin: 0.2em 0; font-size:115%; }
...
table.header { width:100%; z-index:100;
               background-color:#DDD; font-weight:bold; font-size:80%; }
...
const WebRoot web_root{
  "/data/safe/paa/web/talk/", "http://wigclust19.cshl.edu/talk/"};
...
class Talk : public PageElement {
 public:
  ...
  virtual std::string html() const {
    ...
    talk << Header(person.name(),
                   hlink("/talk/", seminar.series()).html(),
                   person.lab()).html();
    ...
    if (page() == "all") {
      talk << Header(seminar.room(),
                     valid,
                     person.employer(), false).html();
...
int main(int argc, char ** argv, char ** envp) {
  try {
    ...
    const InputMethod input{
      std::cin, argc, argv, envp, "/data/safe/paa/web/talk/"};
    ...
    const Seminar seminar{
      "SV, CNV and Phase in Autism, CHD and Cancer",
          "Analysis and Visualization with MUMdex",
          "SNV, CNV, MUMdex",
          "Quantitative&nbsp;Biology&nbsp;Seminar",
          "Simons&nbsp;Center&nbsp;for&nbsp;Quantitative&nbsp;Biology",
          "Hawkins Conference Room", "Wendt Building, Hillside",
          "October 10", "2018"};

    Talk talk{query, "talk2018oct", peter, seminar,
    ...
    talk.slide(seminar, "Introduction", Employees{peter})
          ...
          << type(hlink("http://mumdex.com/talk/"), "red");
...
<p>
Our work examines SV, CNV and phase using data from the autism Simons Simplex Collection (9077 people) and in-house studies on congenital heart disease (537 people) and metastatic breast cancer (202 samples from 67 people).
</p>
<p>
We developed a set of data processing and visualization tools bundled in the MUMdex software package to analyze multiple types of sequence data. The data processing tools include a maximal unique match (MUM) based aligner, a split-read based SV detector, <i>de novo</i> and transmitted mutation finders, a classical CNV pipeline, and software to analyze haplotyping by halves (HAHA) data for phase. The visualization tools include a genome browser for CNV [GGraph], a MUM alignment viewer [mview], and websites to investigate copy-number profiles, <i>de novo</i> indel candidates and detected CNV events.
</p>
<p>
Our findings include a moderate increase in sensitivity and equal specificity for MUMdex <i>de novo</i> indel candidates over a standard method, the near elimination of event-size bias, a good correspondence between large MUMdex indels and CNV-based event calls, and the demonstrated ability to fully and accurately phase an individual with the HAHA method.
</p>
<p>
The seminar will begin with a brief interactive demonstration of a new spatial genomics method.
</p>
</div>
)xxx";
    ...
    talk.slide("This Talk", "Introduction")
        << division(
            type(division(
                heading2("1. a new spatial genomics idea"),
                png("./seating.reconstruct.png", "Spatial Genomics", "35%")),
                 "half"),
            type(division(
                heading2("3. phasing using HAHA (maybe)"),
                png("./haha.png", "HAHA", "45%")), "half"))
        << type(division(
            heading2("2. SV and CNV discovery and visualization"),
            png("./indel_deletion.png", "Indel", "32%"),
            png("./CZ30.png", "", "45%")), "clear");

    talk.slide({"A New Spatial Genomics Idea", "", "Spatial Genomics Idea"},
               "Interlude")
        << type(division(type(numbered(
            "make beads with barcoded poly-A and poly-T oligos",
            "beads are spread on a slide",
            "slide is placed on tissue",
            "poly-A oligos diffuse out",
            "poly-T oligos capture RNA and poly-A oligos",
            "slides are washed and sequenced",
            "reconstruct bead positions from oligo dimer counts",
            "RNA sequence is mapped back to tissue position"),
                              "hugelist")), "left");

    const std::string sg_link{"http://mumdex.com/sg/"};
    talk.slide({"Spatial Genomics Demonstration", "", "Spatial Genomics Demo"},
               "Interlude")
        << heading2("Please go to ", type(hlink(sg_link), "red"),
                    " using your phone or laptop web browser")
        << type(iframe("/talk/sg/index.cgi"), "spatialentry")
        << type(iframe("/talk/sg/graph.html"), "spatial");

    talk.slide({"Spatial Genomics Demonstration", "", "Seating Chart"},
               "Interlude")
        << type(iframe("/talk/sg/graph.html"), "spatialfull")
        << pre("\n\n\n\n\n\n\n\n")
        << type(iframe("/talk/sg/cleanup.cgi"), "spatialfull");

    talk.slide({"Bead Position Reconstruction Simulation", "",
            "Spatial Genomics Simulation"}, "Interlude")
        << png("./seating.true.png", "Bead Position Ground Truth", "46%")
        << png("./seating.pdist.png", "Bead Perturbed Distances",
               "46%")
        << heading("Noise is added to the true distances when simulating");

    talk.slide({"Spatial Genomics Reconstruction"}, "Interlude")
        << png("./seating.reconstruct.png", "Bead Position Reconstruction",
               "46%")
        << png("./seating.rdist.png", "Bead Position Reconstructed Distances",
               "46%")
        << heading("The approximate positions and distances are recovered");

    talk.slide({"Spatial Genomics Ground Truth vs Reconstruction",
            "Simulated Reconstruction", "Spatial Genomics Result"}, "Interlude")
        << png("./seating.true.png", "Bead Position Ground Truth", "46%")
        << png("./seating.reconstruct.png", "Bead Position Reconstruction",
               "46%")
        << heading("Here are the ground truth and reconstruction side-by-side");
    ...
    talk.slide("Copy Number Variant Detection", "Main")
     ...
     talk.slide({"Haplotyping by Halves (HAHA) Method for Phasing", "", "HAHA"},
                "Main")
         << png("./haha.png", "HAHA", "65%")
         << heading2("Recovers full phase for individual from split nuclei "
                     "data, using an HMM");

      talk.slide("HAHA Assessment", "Main")
         << png("./haha_success.png", "HAHA", "49%")
         << png("./haha_overall.png", "HAHA", "49%")
         << heading2("High-quality SNPs are phased with 90% success rate "
                     "out to 10MB")
         << heading2("Most of the genome is phased correctly by the HMM")
         << heading2("On some chromosomes phase is mistakenly flipped "
                     "across centromeres");

      talk.slide("HAHA Afterburner", "Main")
         << png("./haha_measure.png", "HAHA", "60%")
         << heading2("Most flip mistakes are then corrected "
      ...
      talk.slide({"MUMdex and Related Tools From this Seminar",
          ...
          << type(division(
              ...
              heading3("HAHA: HAHA data analysis"),
              list("Software will be available soon")), "half")
          << type(division(png("./indels_web.png", "", "100%"),
                           png("./mview.png", "mview application", "100%"),
                           png("./haha.png", "HAHA software", "100%")),
                  "quarter");
