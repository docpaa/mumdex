...
class InputMethod {
 public:
  ...
  std::string env(const std::string & name) const try {
    ...
    if (name == "REQUEST_URI") return "/mskcc/";
    if (name == "SERVER_NAME") return "mumdex.com";
    if (name == "QUERY_STRING") return "page=0";
    if (name == "SCRIPT_NAME") return "/mskcc/talk.html";
...
class Slide : public PageElement {
 public:
  ...
  virtual std::string html() const {
    ...
    if (!notitle_) slide << heading(title_).html() << "\n";
...
const WebRoot web_root{
  "/data/safe/paa/web/mskcc/", "http://wigtop2.cshl.edu/mskcc/"};
...
int main(int argc, char ** argv, char ** envp) {
  try {
    ...
    const InputMethod input{
      std::cin, argc, argv, envp, "/data/safe/paa/web/mskcc/"};
    ...
    const Seminar seminar{
      "Copy Number Analysis and Visualization",
          "with MUMdex and G-Graph",
          "G-Graph Visualization",
          "Special&nbsp;Seminar",
          "Memorial&nbsp;Sloan&nbsp;Kettering&nbsp;Cancer&nbsp;Center",
          "11th&nbsp;Floor&nbsp;Conference&nbsp;Room", "MSKCC",
          "July 1", "2019"};

    Talk talk{query, "mskcc", peter, seminar,
          png("atan_full.png", "atan Profile", "100%", "atan_full.png")};
    ...
    talk.slide("This Seminar", "Introduction")
        << heading2("1. MUMdex, Copy Number Analysis and SMASH ")
    ...
    talk.slide("What is MUMdex?", "Part 1").center(false)
        << heading2("MUMdex software uses MUMs for genomic analysis "
                    "and visualization")
        << png("./bridges_read.png", "", "100%")
        << heading2("MUMdex features:")
        << type(list(
            "MUMdex is written in C++ and runs on Linux, Mac, Windows",
            "fast suffix array aligner outputs MUMdex alignment format",
            "<i>de novo</i> and transmitted structural variation "
            "mutation finder",
            "empirical bin copy number analysis",
    ...
    talk.slide("Copy Number Method", "Part 1")
        << png("./cn_primer.png", "The CN method", "100%")
        << heading2("typically 100,000 bins of ~30,000 bases "
                    "with ~100 reads each");
    ...
    talk.slide("Marvel Genomics", "Part 1").notitle()
     ...
     talk.slide("G-Graph Tutorial is Online", "Part 2", "G-Graph")
         << type(iframe("http://mumdex.com/ggraph/"), "indels")
         << heading2("G-Graph comes bundled with MUMdex:")
         << type(hlink("https://mumdex.com/ggraph/"), "red");
     ...
     talk.slide("Copy Number Variation in Metastatic Breast Cancer",
                "Part 2", "CN in Breast Cancer")
         << heading2("Website for project uses new CN scale "
                     "to visualize highly aneuploid profiles")
         << type(iframe(cn_link), "indels")
         << heading2("scale choices, zoom to chromosome, "
     ...
     talk.slide("SV and CNV Complementarity in SSC Data",
      ...
      talk.slide("MUMdex and Related Tools",
