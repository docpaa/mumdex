...
// similar_bridge
//
// look for a bridge that is similar to a specific bridge in bridges files
//
...
int main(int argc, char* argv[])  try {
  // Check usage
  if (--argc < 12) {
    throw Error("usage: similar_bridge ref bridges_dir pop_file "
                "chr1 pos1 high1 chr2 pos2 high2 inv dist [sample|family] ...");
  }
  ...
  const unsigned int dist{static_cast<unsigned int>(atoi(argv[11]))};

  if (0) cerr << high1 << high2 << endl;  // to avoid warning
  ...
  while (argc--) {
    ...
    for (const Sample & sample : pop.samples(sample_or_family)) {
      ...
      // Match range
      const unsigned int low_pos1{pos1 > dist ? pos1 - dist : 0};
      const unsigned int high_pos1{pos1 + dist};
      const unsigned int low_pos2{pos2 > dist ? pos2 - dist : 0};
      const unsigned int high_pos2{pos2 + dist};

      // Advance to start position
      FileVector<BridgeInfo>::const_iterator start{
        lower_bound(bridges.begin(), bridges.end(), low_pos1,
