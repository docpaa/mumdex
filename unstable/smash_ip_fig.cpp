...
int main(int argc, char * argv[]) try {
  ...
  PSPage page{ps, output_prefix + " - total maps: " +
        paa::commas(good_mappings.size()), "2 2"};

  // Map counts histogram
  PSGraph map_counts_graph{page, ";Mappings Per Read Pair;N",
  ...
  PSGraph hit_length_graph{page, ";Hit Length;N", Bounds(0, max_read_length)};
  ...
  PSGraph hit_offset_graph{page, ";Hit Offset;N", Bounds(0, max_read_length)};
  ...
  // Mapping distances distribution
  random_device rd;
  auto mersenne = mt19937_64(rd());
  ...
  PSGraph map_dist_graph{page, ";Normalized Inter-Map Distance;N",
        Bounds(0, 1, 0, nunset())};
  PSHSeries<double, uint64_t> map_dist_series{map_dist_graph, 100,
        "1 0 0", false};
  sort(good_mappings.begin(), good_mappings.end(),
       [](const MappingInfo & lhs, const MappingInfo & rhs) {
  ...
  for (const MappingInfo & map : good_mappings) {
    const double dist{(map.position() > last_pos ?
                       map.position() - last_pos :
                       last_pos - map.position()) + unit()};
    const double ddist{dist * good_mappings.size() /
          cn_abspos.n_positions()};
    map_dist_series.add_point(ddist);
    last_pos = map.position();
  }
