...
template <typename C, typename T>
inline
basic_pstreambuf<C, T>::basic_pstreambuf(const std::string& command,
                                         pmode mode)
    : ppid_(-1)   // initialise to -1 to indicate no process run yet.
    , wpipe_(-1)
    , wbuffer_(nullptr)
  ...
  if (!is_open()) {
    switch (fork(mode)) {
      case 0 :
        ...
        ::execl(shell_path, "bash", "-c", command.c_str(), nullptr);
...
basic_pstreambuf<C, T>::showmanyc() {
  ...
  if (sizeof(char_type) == 1)
    avail = static_cast<int>(
        fill_buffer(true) ? this->egptr() - this->gptr() : -1);
#ifdef FIONREAD
...
// typename basic_pstreambuf<CharT, Traits>::fd_type&
inline pstreams::fd_type &
basic_pstreambuf<CharT, Traits>::wpipe() {
  return wpipe_;
}
