...
if [ $# != 7 ] ; then
    echo usage: smash.sh mumdex_dir ref_file bins_dir n_bins,... sample_name fastq1.gz fastq2.gz 1>&2
    exit 1
fi
...
if $mumdex_dir/smash "$ref" <(zcat "$fastq1") <(zcat "$fastq2") 20 4 1000 12 "$name" "$bin_files" "$bad_pos" > "$name".out.txt 2> "$name".err.txt ; then
